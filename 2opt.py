...
def CostCalculation(tour,cost_matrix):
    ...
    DESCRIPCIÓN: Esta función calcula el coste total de una ruta y devuelve su coste.
    
    INPUT: (tour)    - Lista que contiene la secuencia de nodos visitados
           (cost_matrix) - Cost matrix (full) con el coste asociado de moverse desde el nodo i al nodo j 
    
    OUTPUT: (tour_cost)      - Coste total del recorrido de entrada..
    """    
    tour_cost = 0
...
def SwapTwo(tour,i,k):
    ...
    DESCRIPTION:   Esta función recrea un recorrido intercambiando nodos en las posiciones i y k. 
     La función rompe el tour en la posición i-1, reconecta la posición i-1 con la posición k e invierte el tour central para reconectar la posición i con k+1.
    
    INPUT: (tour)      -  Lista que contiene la secuencia de nodos visitados
           (i) - Posición del primer nodo de la lista
           (k) - Posición del segundo nodo de la lista 
               
           
           
    OUTPUT: (new_tour)   - Nuevo recorrido recreado.
    """ 
...
def SwapTwoFast(tour,i,k):
    ...
    DESCRIPTION:   La función rompe el tour en la posición i-1, reconecta la posición i-1 con la posición k e invierte el tour central para reconectar la posición i con k+1.  
...
def TwoOptFast(tour,dist):
    size = len(tour) #duración del viaje
    while True: 
        minchange = 0
